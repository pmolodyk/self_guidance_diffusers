...
So, the main idea of our methodology is to use the remarkable generative abilities of diffusion models (DM) to produce natural-looking adversarial clothes.
Moreover, since we want our method to be zero-shot, that is, to use pre-trained models only, ideally, we would like to be able to use the DM to find a trajectory that would lead us to adversarial samples hidden inside the areas of distribution of natural-looking images.
Here, a trajectory is the denoising path the DM choses when producing the final result starting from random noise. 
So, we would like to be able to change the default generation trajectory of our DM to push it towards enhancing the adversarial effects of the image. DMs are well known for being controllable by mechanisms of attention (e.g. text-conditional generation, implemented in Stable Diffusion)
% todo cite SD paper
 and guidance (e.g. classifier guidance).
% todo cite classifier guidance paper
Therefore, we aim to create a controllable adversarial generation pipeline by modifying the DM sampling process while retaining the DM's natural controllability.
% todo more 
...
As input, we receive a prompt from the user, and generate adversarial clothes with a pattern as close to the original generation of our stable diffusion model as possible as an output.
Again, in our case, adversarial means that the clothes can help avoid detection altogether instead of just making the detector assign an incorrect class to the detected bbox.
At the same time, we also need to ensure that the generated pattern is still effective in the physical world, since our ultimate goal is to be able to print the clothes, put them on and demonstrate how they can be used to avoid detection systems. This means that we need to take special measures so that the patch retains its adversarial nature when deformed and partially obscured in a real-world environment.

First we generate a latent vector that would be used as input for the DM. This vector can be either random or selected by a diffusion inversion method to replicate a user-selected image.
% todo cite diffusion inversion paper 
Again, one of the main advantages of our method is that, unlike other methods, it is able to trade off adversarial effectiveness for more natural-looking patches that can be controlled by the user with the use of the prompt and initial conditioning, which is the chosen latent, our starting point in the noise distribution.
If the user is providing a latent vector that to generate a specific image, it is also necessary to provide a prompt that corresponds to that image. Many models exist that can reverse-engineer a prompt given the input image, the choice of such model is out of the scope of this work.
% todo cite prompt reverse-engineering
After that, we modify the sampling process of the diffusion model, adding ``detection guidance'' to the update in order to ensure that our patch helps us avoid the detector.
In order to obtain this ``detection guidance'' on each step of the sampling process, we need to generate an intermediate image by applying a VAE on the latent space and then simulating a real-life scenario of a person wearing clothes with the generated patch. We then run the detector on the created image, calculate the detection confidence and use the resulting gradient to guide the sampling process. We implemented 2 methods of scenario simulation: a simple 2d pasting algorithm and a more complex 3d-generated scene creator. We will describe both methods in detail in later sections.

We later show that this ``detection guidance'', when used by itself, creates images that are visually jarring and are far by l1-norm from the image, produced by the default sampling. To counter this, we implement other guidance methods, such as ``appearance constraint guidance''. We show that these methods combined yield much better images, and provide us with a valuable instrument of trade-off between the adversarial effectiveness and visual quality of the resulting patch, which we can use by selecting the relative weights of the corresponding guidance gradients.

\subsection{Models}

In order to understand the pipeline in more detail, first, we need to look at the components closely.

Two models are used in this pipeline: a Stable Diffusion model (SD) for generating the patch, and a detector model (we test our method with YOLOv2, YOLOv3 (YOLO), fasterRCNN and mmdet) to ensure that the generated patch is adversarial with respect to detection.
...
Stable Diffusion (SD) is a standard benchmark for text-conditioned image generation. It is a decently fast DM with a variety of pre-trained models widely available. We used 2 versions of the model, MiniSD, which generated images 420x420, and a default SD distribution with a resolution 512x512.
% TODO details on SD
...
\paragraph{Introduction} A pre-trained SD model, just like any other DM, generates trajectories from the noise distribution to the distribution of meaningful for us humans images by gradually removing noise from this random starting point.
% todo rephrase
...
In this process, as we look at the pictures in each row from left to right, we observe the model gradually removing the noise to arrive at different finishing points.
All three final generations in the last column, however, satisfy our conditioning requirement, which is the given prompt.

The actual diffusion process happens in the latent space, and the transition to the image space is only made on the last stage using a decoder model.
During the training process of a DM, the training images are encoded to the latent space, and then noise is gradually added to the latent in a Markov-Chain (forward diffusion process), and then the image is reconstructed in the reverse (backward) diffusion process. This is done by training a U-Net model to predict the noise components on every step, thus simulating the reverse Markov-Chain process.
% todo add formulas from diffusion papers
Thus later, in inference, during sampling, we can use this trained U-Net to perform the backward diffusion, starting from an arbitrary latent vector, to generate an image.

An important part of the process is conditioning, which in case of a SD model is added in the form of a text prompt, or more rarely a semantic map. The text is encoded using the CLIP model and incorporated into the latent vector via cross-attention mechanisms.

The encoder-decoder pair is typically a Variational Autoencoder (VAE): its encoder allows the transition into the latent space during training, while its decoder returns latents to the corresponding images in the pixel space.
...
The transition between the spaces is possible with the use of a VAE with an encoder $\mathcal{E}$ and decoder $\mathcal{D}$.
That is, a real image $x$ can be translated into a latent vector $z$ in the latent space by applying the encoder $\mathcal{E}$ of this VAE, while the reverse process is carried out by the decoder $\mathcal{D}$: 
$$z = \mathcal{E}(x), \Tilde{x} = \mathcal{D}(z)$$

$\Tilde{x}$ is a reconstructed version of the original image $x$. 
During the training of a SD model, the weights of its VAE model are fixed.

The forward diffusion process gradually adds noise to our original latent $z$ in $T$ steps, and the model trains to predict the previous latent $z_{t-1}$ for all $z_t$ in the reverse process.
...
% TODO should I include the loss function for SD? YES, and more formulas on how the forward and backward processes look like!
...
% todo cite relevant paper
On the contrary, since generation is performed with a fixed scheduler, it is impossible to run the cycle for an infinite number of iterations without altering the sampling process of the given pre-trained model.
...
Let us formally describe guidance, a method that can be used to change the distribution from which the sampling will be done by sampling from a conditional distribution on every step of the process.
